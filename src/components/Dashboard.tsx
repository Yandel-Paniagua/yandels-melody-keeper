...
import { Music, Plus, Pencil, Trash2, LogOut, Search, Package, Users, MessageSquare, Truck } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
type TabKey = "inventario" | "clientes" | "resenas" | "envios";
...
const TABS: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "resenas", label: "Reseñas", icon: <MessageSquare className="w-4 h-4" /> },
  ...
  return (
    <div id="dashboard" className="min-h-screen bg-background">
      ...
      <header id="dashboard-header" className="border-b border-border bg-card sticky top-0 z-10">
        ...
        {/* Tab navigation */}
        <div id="tab-nav" className="max-w-7xl mx-auto px-4 sm:px-6">
          <nav id="tab-nav-list" className="flex gap-1 -mb-px overflow-x-auto">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                id={`tab-${tab.key}`}
